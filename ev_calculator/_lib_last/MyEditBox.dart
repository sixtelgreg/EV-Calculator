...
class MyEditBox extends StatefulWidget {
  ...
  MyEditBox.fromDbl(
      {this.title, double value, int digits = 1, Function(double) onChange})
  ...
  MyEditBox.fromInt({this.title, int value, Function(int) onChange})
...
class _MyEditBoxState extends State<MyEditBox> {
  ...
  void initState() {
    ...
    if (null != _inputValueInt) {
      _ctrl.addListener(_inputValueInt);
    }

    if (null != _inputValueDbl) {
